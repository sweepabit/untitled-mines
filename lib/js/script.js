...
if(typeof $.cookie('clientSeed') === "undefined"){
    var weed = Math.floor((Math.random() * 600000) + 1);
    var clientSeed = weed;
    console.log(weed);
}else{
    ...
    socket.on('connect', function() {
        ...
        socket.emit('auth', authRequest, function(err, authResponse) {
            ...
            if(getURLParameter('access_token')!="" && getURLParameter('access_token')!=null){
                ...
                $.getJSON("https://api.moneypot.com/v1/list-bets?access_token="+getURLParameter('access_token')+"&app_id=1135&wager=9900000&limit=11&order_by=desc", function(json){
...
$('#depositButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/deposit?app_id=1135';
...
$('#withdrawButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/withdraw?app_id=1135';
    var windowName = 'manage-auth';
    var windowOpts = 'width=420,height=350,left=100,top=100';
    var windowRef = window.open(windowUrl, windowName, windowOpts);
    windowRef.focus();
...
function onloadCallback() {
    grecaptcha.render('faucetClaimCaptcha', {
        'sitekey' : '6LdmMyATAAAAAKlDBn6Ljtbhi4qYd_cDcPFxE5Nf',
...
function startNewGame(){
    if(!connected){
        $('#theDimmer').html("Error: Not connected!<br><a href=\"https://www.moneypot.com/oauth/authorize?app_id=1135&response_type=token&state=Meh&redirect_uri=http://untitled-mines.github.io/untitled-mines\">Click here to login</a>");
